...
function analisar () {
    ...
    if (insNumero(num.value) && !insLista(num.value,valores)){
        ...
        alert ('Valor Inserido ou Inválido')
    }
...
function finalizar() {
    ...
    if ( total == 0){
        ...
        alert ('Favor inserir entre 1 a 100')  
        ...
        res.innerHTML = ' '
        res.innerHTML += `Foram digitados ${total} números`
        ...
        res.innerHTML += `<p> Os valores digitados na ordem decrescente ${valores.sort((a,b) => b - a ).join(' # ')}`
